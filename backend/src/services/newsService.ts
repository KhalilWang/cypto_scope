...
const mockNewsTemplates: Record<string, NewsTemplate[]> = {
  default: [
    {
      titles: [
...
function analyzeSentiment(
  title: string,
  description: string,
  templateBias: 'bullish' | 'bearish' | 'neutral'
  ...
  if (templateBias === 'bullish') bullishScore += 2;
  if (templateBias === 'bearish') bearishScore += 2;
...
function extractKeywords(title: string, description: string, baseKeyword: string): string[] {
...
export class NewsService {
  ...
  private readonly useMockOnly: boolean = false;
  ...
  async fetchNews(keyword: string, days: number = 7): Promise<NewsArticle[]> {
    ...
    try {
      ...
      return response.data.articles.slice(0, 5).map((article, index) => 
        this.transformRealArticle(article, keyword, index)
      );
    } catch (error) {
      console.warn('Failed to fetch news from NewsAPI, using mock data:', 
  ...
  private generateMockNews(keyword: string, isMock: boolean = true): NewsArticle[] {
    const normalizedKeyword = keyword.toLowerCase().replace(/[^a-z]/g, '');
    
    let templates: NewsTemplate[] = mockNewsTemplates[normalizedKeyword] || mockNewsTemplates.default;
  ...
  async getNewsComparison(
    keyword: string,
    days: number = 7
  ): Promise<NewsComparison> {
    ...
    const sources: NewsComparison['sources'] = [];
  ...
  getAvailableSources(): Array<{ name: string; url: string; region: string; isReal: boolean }> {
    return [

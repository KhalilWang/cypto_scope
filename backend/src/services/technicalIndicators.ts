...
export class TechnicalIndicatorsService {
  ...
  private readonly KDJ_PERIOD = 9;
  private readonly KDJ_SMOOTH_K = 3;
  private readonly KDJ_SMOOTH_D = 3;
  private readonly CCI_PERIOD = 20;
  private readonly ATR_PERIOD = 14;
  private readonly WILLIAMS_R_PERIOD = 14;
  private readonly STOCH_FAST_K_PERIOD = 14;
  private readonly STOCH_SLOW_K_PERIOD = 3;
  private readonly STOCH_SLOW_D_PERIOD = 3;
  ...
  generateTradingSignal(indicators: TechnicalIndicators): TradingSignal {
    ...
    const indicatorsAnalysis: TradingSignal['indicatorsAnalysis'] = {
  ...
  private calculateEMAMethod(prices: number[], period: number): number[] {
    const ema: number[] = [];
  ...
  private calculateEMA(prices: number[]): EMAIndicator {
  ...
  private calculateBollingerBands(prices: number[]): BollingerIndicator {
    ...
    let signal: 'squeeze' | 'expanding' | 'upper_touch' | 'lower_touch' | 'neutral' = 'neutral';
    let interpretation = '';
  ...
  private calculateKDJ(prices: PriceHistoryPoint[]): KDJIndicator {
    ...
    let signal: 'golden_cross' | 'death_cross' | 'overbought' | 'oversold' | 'bullish' | 'bearish' | 'neutral' = 'neutral';
    let interpretation = '';
  ...
  private calculateCCI(prices: PriceHistoryPoint[]): CCIIndicator {
    ...
    let signal: 'overbought' | 'oversold' | 'bullish' | 'bearish' | 'neutral' = 'neutral';
    let interpretation = '';
  ...
  private calculateATR(prices: PriceHistoryPoint[]): ATRIndicator {
    ...
    let signal: 'high_volatility' | 'low_volatility' | 'normal' = 'normal';
    let interpretation = '';
  ...
  private calculateOBV(prices: PriceHistoryPoint[]): OBVIndicator {
    const values: number[] = [];
    let obv = 0;
    ...
    let signal: 'bullish_divergence' | 'bearish_divergence' | 'confirmation' | 'neutral' = 'neutral';
    let interpretation = '';
  ...
  private calculateWilliamsR(prices: PriceHistoryPoint[]): WilliamsRIndicator {
  ...
  private calculateStochastic(prices: PriceHistoryPoint[]): StochasticIndicator {

...
class MarketService {
    constructor() {
        ...
        this.cacheDurationMinutes = config_1.config.cacheDurationMinutes;
    }
    async fetchGlobalData() {
        try {
            const response = await axios_1.default.get(`${this.apiUrl}/global`, { timeout: 15000 });
    ...
    async updateMarketCache(globalData) {
        database_1.db.exec(`
    ...
    `);
        database_1.db.exec('DELETE FROM market_cache');
    ...
    `).run(globalData.total_market_cap.usd, globalData.total_volume.usd, globalData.market_cap_percentage.btc, globalData.market_cap_percentage.eth, globalData.active_cryptocurrencies, globalData.markets, globalData.market_cap_change_percentage_24h_usd);
